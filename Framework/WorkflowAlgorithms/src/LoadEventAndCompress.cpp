...
MatrixWorkspace_sptr LoadEventAndCompress::loadChunk(const size_t rowIndex) {
  ...
  if (!loadLogs) {
    ...
    const bool filterByTimeStart = getProperty("FilterByTimeStart");
    const bool filterByTimeStop = getProperty("FilterByTimeStop");
    const bool filterMonByTimeStart = getProperty("FilterMonByTimeStart");
    const bool filterMonByTimeStop = getProperty("FilterMonByTimeStop");
    loadLogs = filterByTimeStart || filterByTimeStop || filterMonByTimeStart ||
               filterMonByTimeStop;
  }
